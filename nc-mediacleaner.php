...
$available_year_directories = array_slice($attempt('propfind', $files_base_path . NEXTCLOUD_UPLOAD_PATH, [
    '{DAV:}displayname',
...
$file_regexes = [
    '/^(?<century>20)?(?<year>' . join('|', $padleft(range('00', date('y')))) . ')(?<month>' . $months . ')(?<day>[0-2][1-9]|3[0-1])/',
    '/^(?<century>20)?(?<year>' . join('|', $padleft(range('00', date('y')))) . ')(?<sep>[-_:])(?<month>' . $months . ')(?P=sep)(?<day>[0-2][1-9]|3[0-1])/',
    '/^(?<day>[0-2][1-9]|3[0-1])(?<century>20)(?<month>' . $months . ')(?<year>' . join('|', $padleft(range('00', date('y')))) . ')/',
];

foreach ($available_year_directories as $year_directory_id => $available_year_directory) {
    $year = basename($year_directory_id);
    $available_month_directories = array_slice($attempt('propfind', $year_directory_id, [
        '{DAV:}displayname',
        '{DAV:}getcontentlength'
            ], 1), 1);

    foreach ($available_month_directories as $month_directory_id => $available_month_directory) {
        ...
        foreach ($available_media_files as $file_id => $available_media_file) {
            ...
            foreach ($file_regexes as $file_regex) {
                if (preg_match($file_regex, basename($file_id), $matches) !== 1) {
                    continue;
                }

                $expected_location = (empty($matches['century']) ? '20' : $matches['century']) . $matches['year'] . '/' . $matches['month'];
            }
